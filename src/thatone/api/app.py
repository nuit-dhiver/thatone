...
A thin wrapper over :class:`~thatone.engine.ThatOne`. Routes parse
...
def get_engine(request: Request) -> ThatOne:
    engine: ThatOne | None = getattr(request.app.state, "engine", None)
...
EngineDep = Annotated[ThatOne, Depends(get_engine)]


def create_app(settings: Settings | None = None, *, engine: ThatOne | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        app.state.engine = engine or ThatOne.open(settings or Settings())
        try:
            yield
        finally:
    ...
    @app.exception_handler(ThatOneError)
    async def handle_domain_error(_: Request, exc: ThatOneError) -> JSONResponse:
