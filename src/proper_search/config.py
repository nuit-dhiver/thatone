...
so ``PROPER_SEARCH__VISION__MODEL=claude-opus-5`` sets ``vision.model``.
...
ENV_PREFIX = "PROPER_SEARCH__"
DEFAULT_CONFIG_FILENAMES = ("proper-search.yaml", "proper-search.yml", ".proper-search.yaml")
...
class StorageSettings(BaseModel):
    ...
    path: Path = Path("./proper-search.db")
    ...
    blob_dir: Path = Path("./proper-search-blobs")
...
class FetchSettings(BaseModel):
    ...
    user_agent: str = "proper-search/0.1"
