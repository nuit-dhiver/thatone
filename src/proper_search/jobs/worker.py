...
* :class:`~proper_search.errors.RetryableError` — back off and requeue. Rate
  limits, timeouts, 5xx.
* :class:`~proper_search.errors.TerminalError` — record and move on. A refusal,
  a corrupt file, a malformed request. Retrying produces the identical failure
  and burns the budget doing it.
* :class:`~proper_search.errors.AuthError` — stop the worker. Bad credentials
...
class Worker:
    ...
    async def _run_job(self, job: Job) -> None:
        ...
        try:
            ...
            raise
        except (ProperSearchError, OSError) as exc:
